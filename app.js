...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initDashboard() {
        ...
        if (totalCandidatesEl) totalCandidatesEl.textContent = mockCandidates.length;

        const verifiedCount = mockCandidates.filter(c => c.certVerified).length;
        ...
        if (verifiedRateEl) verifiedRateEl.textContent = Math.round((verifiedCount / mockCandidates.length) * 100) + '% success rate';

        const candidatesChangeEl = document.getElementById('candidatesChange');
        if (candidatesChangeEl) candidatesChangeEl.textContent = `${mockCandidates.length} this month`;

        // Populate top candidates
        const topCandidatesList = document.getElementById('topCandidatesList');
        if (topCandidatesList) {
            topCandidatesList.innerHTML = '';
            
            mockCandidates.slice(0, 4).forEach(candidate => {
                ...
                item.innerHTML = `
                    ...
                    <div class="candidate-info">
                        ...
                        <div class="candidate-education">${candidate.education.split(' - ')[1] || ''}</div>
                    </div>
                ...
                item.addEventListener('click', () => {
                    showPage('candidates');
                    const navLink = document.querySelector('[data-page="candidates"]');
                    if (navLink) navLink.classList.add('active');
                });
                topCandidatesList.appendChild(item);
            });
        }
        ...
        if (activityLog) {
            ...
            const activities = [
                { type: 'success', icon: 'fas fa-check-circle', title: 'Certificate Verified', desc: 'Alice Johnson - MS Computer Science', time: '2 hours ago' },
                { type: 'info', icon: 'fas fa-user-plus', title: 'New Candidate', desc: 'David Chen applied for Frontend Developer', time: '5 hours ago' },
                { type: 'success', icon: 'fas fa-check-circle', title: 'Certificate Verified', desc: 'Bob Smith - MS AI & ML', time: '1 day ago' },
                { type: 'info', icon: 'fas fa-file-upload', title: 'Resume Parsed', desc: '3 new resumes processed successfully', time: '2 days ago' }
            ];
    ...
    function initMatchChart() {
        ...
        // if Chart is not available, just skip
